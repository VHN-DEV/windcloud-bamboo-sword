...
function buildPillVisualMarkup(item, qualityConfig, options = {}) {
    ...
    const isNguLinhFlameArt = item.category === 'FLAME_ART' && item.uniqueKey === 'NGU_LINH_THUAT';
    ...
    const isHuThienArtifact = item.category === 'ARTIFACT' && item.uniqueKey === 'HU_THIEN_DINH';
    ...
    if (isNguLinhFlameArt) {
        visualClasses.push('is-flame-art-ngu-linh');
    }
    ...
    const centerMarkup = isPhongLoiArtifact
        ...
        : isNguLinhFlameArt && uniqueConfig?.imagePath
        ? buildItemImageVisualMarkup(uniqueConfig.imagePath, {
            coreClass: 'pill-visual__core--book',
            imageClass: 'pill-visual__item-icon--skill'
        })
        ...
        : isHuThienArtifact && artifactConfig?.imagePath
        ? buildStaticArtifactImageVisualMarkup(artifactConfig.imagePath, 'is-hu-thien-dinh')
        : item.category === 'ARTIFACT' && artifactConfig?.imagePath
        ? buildStaticArtifactImageVisualMarkup(artifactConfig.imagePath)
